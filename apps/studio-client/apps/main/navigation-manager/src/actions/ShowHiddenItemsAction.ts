...
interface ShowHiddenItemsActionConfig extends Config<ContentAction>, Partial<Pick<ShowHiddenItemsAction,
  "treeRelation"
>> {
}
...
class ShowHiddenItemsAction extends ContentAction {
  ...
  constructor(config: Config<ShowHiddenItemsAction> = null) {
    ...
    super(cast(ShowHiddenItemsAction, ActionConfigUtil.extendConfiguration(
      resourceManager.getResourceBundle(null, NavigationTreeLabels_properties).content, { handler: bind(this$, this$.#showHideItems) }, NavigationTreeRelation.ACTION_ID_SHOW_HIDDEN, config)));
  }
  ...
  #showHideItems(): void {
    ...
    editorContext._.getPreferences().getType().addBooleanProperty(ShowHiddenItemsAction.VIEW_TREE_SETTINGS_HIDDEN, showHiddenItems);
  }
